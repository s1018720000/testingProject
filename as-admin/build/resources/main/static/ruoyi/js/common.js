...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        window.parent.$.modal.loading(i18n('home.data.loading'));
...
function checkpwd(chrtype, password) {
    if (chrtype == 1) {
        if(!$.common.numValid(password)){
            $.modal.alertWarning(i18n('user.number.pwd'));
            return false;
        }
    } else if (chrtype == 2) {
        if(!$.common.enValid(password)){
            $.modal.alertWarning(i18n('user.letter.pwd'));
            return false;
        }
    } else if (chrtype == 3) {
        if(!$.common.enNumValid(password)){
            $.modal.alertWarning(i18n('user.letter.number.pwd'));
            return false;
        }
    } else if (chrtype == 4) {
...
$.ajaxSetup({
    complete: function(XMLHttpRequest, textStatus) {
        if (textStatus == 'timeout') {
            $.modal.alertWarning(i18n('home.server.time.out'));
            $.modal.enable();
            $.modal.closeLoading();
        } else if (textStatus == "parsererror" || textStatus == "error") {
            $.modal.alertWarning(i18n('home.server.error'));
            $.modal.enable();
            $.modal.closeLoading();
        }
    }

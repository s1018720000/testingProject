    ...
    $.extend({
    	...
    	table: {
            	...
            	$(optionsIds).on("click", '.tooltip-show', function() {
                    ...
                    if ($.common.equals("copy", target)) {
                        ...
                        parent.layer.alert(input.val(), {
                            title: i18n('home.info.content'),
                            shadeClose: true,
                            btn: [i18n('home.confirm')],
            ...
            exportExcel: function(formId) {
                table.set();
                $.modal.confirm(i18n('home.sure.export') + ' ' + table.options.modalName + ' ' + i18n('home.sure.doubt'), function() {
                    var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                    var params = $("#" + table.options.id).bootstrapTable('getOptions');
                    var dataParam = $("#" + currentId).serializeArray();
                    dataParam.push({ "name": "orderByColumn", "value": params.sortName });
                    dataParam.push({ "name": "isAsc", "value": params.sortOrder });
                    $.modal.loading(i18n('home.exporting'));
            ...
            importExcel: function(formId, width, height) {
            	...
            	layer.open({
                    ...
                    maxmin: true,
                    shade: 0.3,
                    title: i18n('home.import') + ' ' + table.options.modalName + ' ' + i18n('home.data'),
                    content: $('#' + currentId).html(),
                    btn: ['<i class="fa fa-check"></i>'+ i18n('home.import'), '<i class="fa fa-remove"></i>' + i18n('home.cancel')],
                    // 弹层外区域关闭
                    shadeClose: true,
                    btn1: function(index, layero){
                        var file = layero.find('#file').val();
                        if (file == '' || (!$.common.endWith(file, '.xls') && !$.common.endWith(file, '.xlsx'))){
                            $.modal.msgWarning(i18n('home.xls.or.xlsx'));
                            return false;
                        }
            ...
            selectDictLabel: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
                    if (dict.dictValue == ('' + value)) {
                    	...
                    	actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, i18n(dict.dictLabel)));
                        return false;
                    }
                });
                return actions.join('');
    	...
    	modal: {
            ...
            confirm: function (content, callBack) {
                layer.confirm(content, {
                    icon: 3,
                    title: i18n('home.system.info'),
                    btn: [i18n('home.confirm'), i18n('home.cancel')]
                }, function (index) {
                    layer.close(index);
                    callBack(true);
                });
            ...
            open: function (title, url, width, height, callback) {
            	...
            	layer.open({
            	    ...
            	    title: title,
            	    content: url,
            	    btn: [i18n('home.confirm'), i18n('home.close')],
            	    // 弹层外区域关闭
            	    shadeClose: true,
            	    yes: callback,
            ...
            openOptions: function (options) {
            	...
            	var _title = $.common.isEmpty(options.title) ? i18n('home.system.win') : options.title;
                var _width = $.common.isEmpty(options.width) ? "800" : options.width; 
                var _height = $.common.isEmpty(options.height) ? ($(window).height() - 50) : options.height;
                var _btn = ['<i class="fa fa-check"></i>' + i18n('home.confirm'), '<i class="fa fa-close"></i>' + i18n('home.close')];
            	// 如果是移动端，就使用自适应大小弹窗
            	if ($.common.isMobile()) {
            	    _width = 'auto';
            	    _height = 'auto';
        ...
        operate: {
            ...
            submit: function(url, type, dataType, data, callback) {
                var config = {
                    ...
                    data: data,
                    beforeSend: function () {
                        $.modal.loading(i18n('home.processing'));
                    },
                    success: function(result) {
            ...
            detail: function(id, width, height) {
            	...
            	var options = {
                    title: table.options.modalName + ' ' + i18n('home.details'),
                    width: width,
                    height: height,
                    url: _url,
            ...
            detailTab: function(id) {
            	table.set();
            	$.modal.openTab(i18n('home.details') + ' ' + table.options.modalName, $.operate.detailUrl(id));
            },
            // 详细访问地址
            ...
            remove: function(id) {
            	table.set();
            	$.modal.confirm(i18n('home.sure.delete') + ' ' + table.options.modalName + ' ' + i18n('home.sure.info'), function() {
            ...
            clean: function() {
            	table.set();
            	$.modal.confirm(i18n('home.sure.empty') + ' ' + table.options.modalName + ' ' + i18n('home.sure.doubt'), function() {
            ...
            addTab: function (id) {
            	table.set();
                $.modal.openTab(i18n('home.add') + ' ' + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息 全屏
            addFull: function(id) {
            	table.set();
                $.modal.openFull(i18n('home.add') + ' ' + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加访问地址
            addUrl: function(id) {
            ...
            edit: function(id) {
            	...
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            	    ...
            	    $.modal.open(i18n('home.modify') + ' ' + table.options.modalName, url);
            	} else {
            	    $.modal.open(i18n('home.modify') + ' ' + table.options.modalName, $.operate.editUrl(id));
            	}
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            	table.set();
            	$.modal.openTab(i18n('home.modify') + ' ' + table.options.modalName, $.operate.editUrl(id));
            },
            // 修改信息 全屏
            editFull: function(id) {
            	...
            	$.modal.openFull(i18n('home.modify') + ' ' + table.options.modalName, url);
            },
            // 修改访问地址
            editUrl: function(id) {
            	var url = "/404.html";
            ...
            saveSuccess: function (result) {
            	if (result.code == web_status.SUCCESS) {
            	    $.modal.msgReload(i18n('home.save.success'), modal_status.SUCCESS);
                } else if (result.code == web_status.WARNING) {
            	    $.modal.alertWarning(result.msg)
                }  else {
            	    $.modal.alertError(result.msg);
        ...
        tree: {
            ...
            notAllowParents: function(_tree) {
                ...
                if(nodes.length == 0){
                    $.modal.msgError(i18n('home.select.node'));
                    return false;
                }
                for (var i = 0; i < nodes.length; i++) {
                    if (nodes[i].level == 0) {
                        $.modal.msgError(i18n('home.not.root.node') + "（" + nodes[i].name + "）");
                        return false;
                    }
                    if (nodes[i].isParent) {
                        $.modal.msgError(i18n('home.not.father.node') + "（" + nodes[i].name + "）");
                        return false;
                    }
                }
                return true;
            ...
            notAllowLastLevel: function(_tree) {
                ...
                for (var i = 0; i < nodes.length; i++) {
                    if (!nodes[i].isParent) {
                        $.modal.msgError(i18n('home.not.last.node') + "（" + nodes[i].name + "）");
                        return false;
                    }
                }
                return true;
